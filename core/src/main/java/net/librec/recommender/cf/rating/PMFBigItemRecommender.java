...
/**
 * @author szkb
 * @date 2018/12/16 11:57
 */

public class PMFBigItemRecommender extends MatrixFactorizationRecommender {
...
//            simSum += Math.abs(similarity[userIdx][simUserEntry.getKey()]);
    ...
    HashMap<Integer, List<Map.Entry<Integer, Double>>> itemTagSimilarity = new HashMap<>(CAPACITY);
    ...
    private void rankItemTagSimilarityList(double[][] similarityItem) {
        for (int i = 0; i < numItems; i++) {
            ...
            List<Map.Entry<Integer, Double>> listTemp = new ArrayList<>();
            for (int k = 0; k < list.size(); k++) {
                if (k < 20) {
            ...
            itemTagSimilarity.put(i, listTemp);
...
//    }
    ...
    HashMap<String, Double> meanItemTagNumber = new HashMap<>(CAPACITY);
